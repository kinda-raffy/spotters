...
void publish_body_odom(
    Sophus::SE3f Twb_SE3f,
    Eigen::Vector3f Vwb_E3f, Eigen::Vector3f ang_vel_body, ros::Time msg_time) {
...
void publish_all_points(std::vector<ORB_SLAM3::MapPoint*> map_points, ros::Time msg_time) {
    // std_msgs::Bool big_change_msg;
    // big_change_msg.data = false;
    // bool map_has_changed = pSLAM->MapChanged();
    // if (map_has_changed) {
    //     std::cout << ">> Map changed " << map_has_changed << std::endl;
    //     big_change_msg.data = true;
    // }
    // else {
    //     // std::cout << ">>>>>> Map has no change " << map_has_changed << std::endl;
    // }
    // big_change_pub.publish(big_change_msg);
...
Sophus::SE3f tfTransform_to_SE3f(tf::Transform T_tf) {
    tf::Matrix3x3 R_tf = T_tf.getBasis();
    tf::Vector3 t_tf = T_tf.getOrigin();
    Eigen::Matrix3f R_mat;
    R_mat << R_tf[0][0], R_tf[0][1], R_tf[0][2],
             R_tf[1][0], R_tf[1][1], R_tf[1][2],
             R_tf[2][0], R_tf[2][1], R_tf[2][2];
    Eigen::Vector3f t_vec(t_tf.x(), t_tf.y(), t_tf.z());
    return Sophus::SE3f(R_mat, t_vec);
}
