...
def plot_alpha_shape(ax, alpha_shape, colour):
    try:
    ...
    for geom in geoms:
        _plot_polygon(ax, geom, colour)


def _plot_polygon(ax, polygon, colour):
    """
    ...
    patch = PathPatch(path, facecolor=colour, lw=0.8, alpha=0.5, ec="b")
    ax.add_patch(patch)